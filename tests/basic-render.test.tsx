...
Deno.test("can render an intrinsic JSX element", () => {
  const jsx = <div class="foo">Hello world</div>;
  expect(jsx).to.equal("<div class=\"foo\">Hello world</div>");
});

Deno.test("can render an intrinsic JSX element with children", () => {
  const jsx = <div class="foo"><span>Hello world</span></div>;
  expect(jsx).to.equal("<div class=\"foo\"><span>Hello world</span></div>");
});

Deno.test("can render a custom JSX element and remove falsy attrs", () => {
...
Deno.test("can render a custom JSX element without children", () => {
  interface ButtonProps {
    primary: boolean;
  }
  function Button(props: PropsWithChildren<ButtonProps>): string {
...
Deno.test("can render a void element with boolean property", () => {
